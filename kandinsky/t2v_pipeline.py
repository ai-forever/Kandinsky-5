...
class Kandinsky5T2VPipeline:
        ...
        messages = [
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": f"""You are a prompt beautifier that transforms short user video descriptions into rich, detailed English prompts specifically optimized for video generation models.
    ...
    def __call__(
            ...
            seed: int = None,
            num_steps: int = 50,
            guidance_weight: float = 5.,
            scheduler_scale: float = 10.,
                ...
                if save_path is not None:
                    ...
                    if len(save_path) == len(images):
                        for path, video in zip(save_path, images):
                            torchvision.io.write_video(path, video.float().permute(1, 2, 3, 0).cpu().numpy(), fps=24, options = {"crf": "5"})
                return images
