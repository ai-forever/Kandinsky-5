import torch
from transformers import Qwen2_5_VLForConditionalGeneration, AutoProcessor, CLIPTextModel, CLIPTokenizer
...
class ClipTextEmbedder:
    ...
    def __call__(self, texts):
        inputs = self.tokenizer(
            texts, max_length=self.max_length, truncation=True, add_special_tokens=True, padding='max_length', return_tensors='pt'
...
class Qwen2_5_VLTextEmbedder:
    ...
    def __init__(self, conf, device):
        self.model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
            conf.checkpoint_path, torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2", device_map=device
        )
        self.model = freeze(self.model)
    ...
    def __call__(self, texts, type_of_content="video"):
        ...
        inputs = self.processor(
            text=full_texts, images=None, videos=None, max_length=max_length, truncation=True, return_tensors="pt", padding=True
        ).to(self.model.device)

        with torch.no_grad():
            embeds = self.model(
                input_ids=inputs['input_ids'], return_dict=True, output_hidden_states=True
...
class Kandinsky5TextEmbedder:
    ...
    def __init__(self, conf, device="cpu"):
        if 'qwen' in conf.keys():
            self.embedder = Qwen2_5_VLTextEmbedder(conf.qwen, device)
            self.model_name = 'qwen'
        # elif 'givi' in conf.keys():
        #     from .giga_vision import GigaVisionEmbedder
        #     self.embedder = GigaVisionEmbedder(conf.givi)
        #     self.embedder.to(device)
        #     self.model_name = 'givi'
