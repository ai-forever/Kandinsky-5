...
def parallelize_dit(model, tp_mesh):
    if tp_mesh.size() > 1:
        ...
        for i, visual_transformer_block in enumerate(model.visual_transformer_blocks):
                plan = {
                    ...
                    "cross_attention.query_norm": SequenceParallel(sequence_dim=0, use_local_output=True),
                    "cross_attention.key_norm": SequenceParallel(sequence_dim=0, use_local_output=True),
