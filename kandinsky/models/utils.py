...
def mean_var_len(x, cu_seqlens, dim=0):
    return torch.cat(
        [
            local_x.mean(dim=dim)
            for local_x in torch.split(x, torch.diff(cu_seqlens).tolist(), dim=0)
        ],
        dim=0,
    )
...
def sta_block_mask(
    ...
    def sta_l(b, h, y, x):
        ws = window_size // 2
        return (y - x).abs() <= ws + (y == 0) * (x == ws + 1) + (y == s - 1) * (
            x == s - ws - 2
        )
...
def create_doc_causal_mask(
    T: int, H: int, W: int, seq: IntTensor = None, causal: bool = False
) -> None:
    d, HW = torch.diff(seq), H * W
    m = (
        torch.eye(d.numel(), device=seq.device)
...
@torch.compile()
def fast_doc_causal(
    seq: IntTensor = None, causal: bool = False, device="cuda"
) -> Tensor:
    d = torch.diff(seq)
    m = (
...
def nablaT_v2_doc(
    ...
    assert (
        method == "topcdf" or method == "topk"
    ), "nabla method should be topcdf or topk"
    # Map estimation
    B, h, S, D = q.shape
    ...
    if add_sta:
        sta = (
            fast_sta_nabla(T, H, W, wT, wH, wW, device=device)
            .unsqueeze_(0)
            .unsqueeze_(0)
        )
...
def nablaT_v2_doc_mfcausal(
    ...
    doc2 = (
        torch.eye(cl.numel(), dtype=torch.bool, device=device)
        .repeat_interleave(cl, dim=0)
        .repeat_interleave(cl, dim=1)
    )
    doc = (
        torch.logical_or(doc1, doc2)
        .repeat_interleave(H * W, dim=0)
        .repeat_interleave(H * W, dim=1)
    )
    map += doc.log()
    map = torch.softmax(map / math.sqrt(D), dim=-1)
