...
def get_velocity(
    ...
    pred_velocity = dit(
        x, text_embeds["text_embeds"], text_embeds["pooled_embed"], t * 1000, 
        visual_cu_seqlens, text_cu_seqlens,
        visual_rope_pos, text_rope_pos,
        scale_factor=conf.metrics.scale_factor,
        sparse_params=sparse_params
    )
    uncond_pred_velocity = dit(
        x, null_text_embeds["text_embeds"], null_text_embeds["pooled_embed"], t * 1000, 
        visual_cu_seqlens, null_text_cu_seqlens,
        visual_rope_pos, null_text_rope_pos,
        scale_factor=conf.metrics.scale_factor,
        sparse_params=sparse_params
    )
...
def generate(
    ...
    sparse_params = get_sparse_params(conf, {"visual":img}, {"visual_rope": visual_cu_seqlens}, device)
    ...
    for timestep, timestep_diff in list(zip(timesteps[:-1], torch.diff(timesteps))):
        ...
        pred_velocity = get_velocity(
            model, model_input, time, text_embeds, null_text_embeds, 
            visual_cu_seqlens, text_cu_seqlens, null_text_cu_seqlens,
            visual_rope_pos, text_rope_pos, null_text_rope_pos,
            guidance_weight, conf, block_mask=None, torch_mask=None,
            sparse_params=sparse_params
        )
...
def generate_sample( #TODO: simplify??
    ...
    with torch.no_grad():
        bs_text_embed, text_cu_seqlens = text_embedder.encode(captions, type_of_content=type_of_content)
        bs_null_text_embed, null_text_cu_seqlens = text_embedder.encode([negative_caption] * len(captions), type_of_content=type_of_content)
    ...
    visual_cu_seqlens = duration * torch.arange(bs + 1, dtype=torch.int32, device=device)
    visual_rope_pos = [
        torch.cat([torch.arange(end) for end in torch.diff(visual_cu_seqlens).cpu()]),
        torch.arange(shape[-3] // conf.model.dit_params.patch_size[1]), torch.arange(shape[-2] // conf.model.dit_params.patch_size[2])
    ]
    ...
    with torch.no_grad():
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            latent_visual = generate(
                dit, device, (bs * duration, height, width, dim), num_steps, 
                bs_text_embed, bs_null_text_embed, 
                visual_cu_seqlens, text_cu_seqlens, null_text_cu_seqlens,
                visual_rope_pos, text_rope_pos, null_text_rope_pos,
                guidance_weight, scheduler_scale, conf, seed=seed
            )
